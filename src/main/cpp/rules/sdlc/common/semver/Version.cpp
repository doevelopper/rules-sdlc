...
Version::Version(
    const std::uint8_t major, const std::uint8_t minor, const std::uint8_t patch, ReleaseLevel releaseType,
    std::uint8_t tweak)
{
    LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
    m_major = major;
...
Version::Version(
    std::uint8_t major, std::uint8_t minor, std::uint8_t patch, const std::string & extra, const std::string & build)
{
    LOG4CXX_TRACE(
        logger, __LOG4CXX_FUNC__ << " " << std::to_string(major) << " " << std::to_string(minor) << " "
                                 << std::to_string(patch) << " " << extra << " " << build);
...
void Version::set(const std::string & version)
{
    ...
    {
        this->set(
            std::atoi(items_match[1].str().c_str()), std::atoi(items_match[2].str().c_str()),
            std::atoi(items_match[3].str().c_str()), items_match[4], items_match[5]);
    }
    else
    {
...
bool Version::isEquals(const Version & rhs) const
{
    LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
    return (
        this->m_major == rhs.m_major && this->m_minor == rhs.m_minor && this->m_patch == rhs.m_patch
...
std::string Version::maturity() const noexcept
{
    LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
...
const std::string Version::getBuild() const
{
    LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
    return (m_build);
}

const bool Version::isStable() const
{
    LOG4CXX_TRACE(logger, __LOG4CXX_FUNC__);
    return (true);
}
