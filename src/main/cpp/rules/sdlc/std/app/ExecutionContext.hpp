...
class SDLC_API_EXPORT ExecutionContext : public boost::asio::execution_context
{
    ...
    public:
        ...
        int
            priority ( ) const;
        friend bool
            operator< ( const QueuedHandleBase & lhs, const QueuedHandleBase & rhs ) noexcept
        {
    ...
    struct derefLess
    {
        ...
        bool
            operator( ) ( const Pointer & a, const Pointer & b ) noexcept ( noexcept ( *a < *b ) )
        {
            return *a < *b;
        }
    };
...
public:
    ...
    public:
        ...
        void
            dispatch ( Function f, const Allocator & ) const
        {
        ...
        template < typename Function, typename Allocator >
        void
            post ( Function f, const Allocator & ) const
        {
            LOG4CXX_TRACE ( logger, __LOG4CXX_FUNC__ );
            m_context.add ( m_priority, std::move ( f ) );
        }
        template < typename Function, typename Allocator >
        void
            defer ( Function f, const Allocator & ) const
        {
            LOG4CXX_TRACE ( logger, __LOG4CXX_FUNC__ );
            m_context.add ( m_priority, std::move ( f ) );
        }
        ...
        bool
            operator== ( const Executor & rhs ) const noexcept
        {
            LOG4CXX_TRACE ( logger, __LOG4CXX_FUNC__ );
    ...
    ExecutionContext &
        operator= ( const ExecutionContext & ) = default;
    ExecutionContext &
        operator= ( ExecutionContext && ) = default;
    ...
    template < typename F >
    void
        add ( int priority, F function )
    {
        LOG4CXX_TRACE ( logger, __LOG4CXX_FUNC__ );
        std::unique_ptr< QueuedHandleBase > handler (
            new QueuedHandler< F > ( priority, --m_order, std::move ( function ) ) );
