...
void GTestEventListener::OnTestCaseStart(const testing::TestCase& test_case )
{
    ...
    LOG4CXX_TRACE(logger, "    " << ::testing::UnitTest::GetInstance()->current_test_case()->name() << "    " << test_case.test_to_run_count() << "/" << test_case.total_test_count());
    LOG4CXX_TRACE(logger, ">--------------->>--------------->>--------------->>--------------->>--------------->");
}
...
void GTestEventListener::OnTestCaseEnd(const testing::TestCase& test_case)
{
    ...
    LOG4CXX_TRACE(logger, "<---------------<<---------------<<---------------<<---------------<<--------------- ");
    LOG4CXX_TRACE(logger, "    " << ::testing::UnitTest::GetInstance()->current_test_case()->name() << "   " << test_case.elapsed_time() << " ms" );
    LOG4CXX_TRACE(logger,
        " Passed(" << test_case.successful_test_count() << ")" <<
...
{
    LOG4CXX_INFO(logger, "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
    LOG4CXX_INFO(logger, "Total Test Case: "
...
{
    // std::chrono::milliseconds ut_ms = std::chrono::duration_cast<std::chrono::milliseconds>(unit_test.elapsed_time());
    // LOG4CXX_INFO(logger, unit_test.Passed());
    LOG4CXX_INFO(logger, "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
    LOG4CXX_INFO(logger, unit_test.reportable_test_count()
                        << " tests Executed: Passed(" << unit_test.successful_test_count()
...
void GTestEventListener::OnTestPartResult( const testing::TestPartResult& test_part_result)
{
    ...
    if (test_part_result.failed())
    {
        ...
        LOG4CXX_WARN(logger, status
                ...
                << " Message: " << test_part_result.message()
